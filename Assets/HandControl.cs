	...
	void Update(){
		...
		if (Physics.Raycast (transform.position, Vector3.forward, out hit)) {
			...
			print (hit.collider.tag);
				...
				StartCoroutine(delayforMenu(txt_quit)); 
						...
						} else 	if (hit.collider.tag == "menuItemHelp") {
				loadValue=2;
							StartCoroutine(delayforMenu(txt_help));  
		...
		if (elbowBone.transform.position.y < handBone.transform.position.y) {
			...
			if (handBone.transform.position.z > currentZ+0.1f){
				if(loadValue==0)Application.LoadLevel("main_game");
				else if(loadValue==1){print ("Quittung");Application.Quit();}
				else if(loadValue==2){print ("Inst<<<<<<<<<<<");Application.LoadLevel("instructions");}
			}
